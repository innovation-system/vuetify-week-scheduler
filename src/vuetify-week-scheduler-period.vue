		...
		>
			<div class="vws-period-container">
				...
				<div class="vws-period-time">
					...
					<span class="text--caption ml-2" v-show="shortPeriod">{{ options.title }}</span>
				</div>
...
export default {
	...
	methods: {
		handleDrag() {
			...
			this.addListener(el, 'click', onClick)
		},
		handleResize() {
			const el = this.$el
			...
			const onMove = e => {
				...
				if (shouldResize) {
					...
					const dragDelta = this.roundBlock(startDrag - this.$parent.getY(e, true))
